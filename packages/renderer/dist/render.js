...
import { h, nextTick as vueNextTick, ref, shallowRef, watch } from 'vue';
import { createApp } from "./renderer.js";
import { createNode, emitLayoutListeners, Output, renderNodeToOutput, renderNodeToScreenReaderOutput, renderStaticSubtrees, hasStaticContent, } from '@vue-ink/core';
import { createInputManager } from "./input.js";
import { createFocusManager } from "./focus-context.js";
import { BSU, ESU } from "./write-synchronized.js";
import { APP_CONTEXT_KEY, STDIN_CONTEXT_KEY, STDOUT_CONTEXT_KEY, STDERR_CONTEXT_KEY, ACCESSIBILITY_CONTEXT_KEY, FOCUS_CONTEXT_KEY, ANIMATION_CONTEXT_KEY, CURSOR_CONTEXT_KEY, } from "./context.js";
import { createAnimationScheduler } from "./animation-scheduler.js";
import { enableKittyKeyboard, disableKittyKeyboard, hasCompleteKittyQueryResponse, stripKittyQueryResponses, } from "./kitty-keyboard.js";
const isCiEnv = () => {
...
export const _flushActiveInstances = async () => {
    ...
    for (const method of CONSOLE_METHODS) {
        ...
        console[method] = ((...args) => {
            ...
            for (const sub of consoleSubscribers) {
                if (isStderrChannel)
                    sub.writeStderr(text);
                else
                    sub.writeStdout(text);
            }
        });
    }
};
const uninstallConsolePatch = () => {
    if (!originalConsoleMethods)
        return;
    for (const method of CONSOLE_METHODS) {
...
const render = (component, options = {}) => {
    ...
    const renderThrottleMs = debug || maxFps === Number.POSITIVE_INFINITY ? 0 : Math.max(1, Math.ceil(1000 / maxFps));
    const existing = instances.get(writeStream);
    if (existing) {
        stderr.write('Warning: render() was called again for the same stdout before the previous instance was unmounted. Reusing stdout across multiple render() calls is unsupported. Call unmount() first.\n');
    ...
    const { destroy: destroyFocusManager, ...focusContext } = createFocusManager(inputManager.emitter);
    ...
    const writeAboveFrame = (target, data) => {
        if (unmounted)
            return;
    ...
    const commitWaiters = [];
    const drainCommitWaiters = () => {
        if (commitWaiters.length === 0)
            return;
    ...
    const scheduleTrailingRender = (delay) => {
        if (trailingTimer !== undefined)
            return;
        trailingTimer = setTimeout(() => {
            trailingTimer = undefined;
            if (unmounted || !hasPendingRender)
                return;
            commitNow();
        }, Math.max(0, delay));
    };
    ...
    app.provide(STDOUT_CONTEXT_KEY, {
        stdout: writeStream,
        write: (data) => writeAboveFrame(writeStream, data),
    });
    app.provide(STDERR_CONTEXT_KEY, {
        stderr,
        write: (data) => writeAboveFrame(stderr, data),
    });
    const isScreenReaderEnabled = ref(options.isScreenReaderEnabled ?? process.env['INK_SCREEN_READER'] === 'true');
    ...
    const buildCursorSuffix = (visibleLineCount, position) => {
        if (!position)
            return '';
    ...
    const buildReturnToBottom = (previousLineCount, previousPosition) => {
        if (!previousPosition)
            return '';
    ...
    const doRender = () => {
        ...
        const { output: text, staticOutput } = renderTree(rootNode, getTerminalWidth(), isScreenReaderEnabled.value);
        ...
        if (debug) {
            if (newStaticFrame.length > 0)
                fullStaticOutput += newStaticFrame;
            writeStream.write(`${fullStaticOutput}${text}\n`);
            emitRenderMetrics(text, text.length === 0 ? 0 : text.split('\n').length, startedAt);
            drainCommitWaiters();
            return;
        }
        const cursorChanged = cursorPosition?.x !== lastCursorPosition?.x ||
        ...
        if (useTTYFrame) {
            ...
            if (incrementalRendering &&
                ...
                writeStream.write(`${openSync}${returnPrefix}${buildIncrementalDiff(lastOutputLines, nextLines)}${cursorSuffix}${closeSync}`);
            }
            else {
                const erase = lastLineCount > 0 ? ansiEscapes.eraseLines(lastLineCount + 1) : '';
                writeStream.write(`${openSync}${returnPrefix}${erase}${newStaticFrame}${text}\n${cursorSuffix}${closeSync}`);
            }
        }
    ...
    const buildIncrementalDiff = (previousLines, nextLines) => {
    ...
    const emitRenderMetrics = (text, lineCount, startedAt) => {
        if (!hasOnRender)
            return;
        frameCounter += 1;
        try {
    ...
    if (kittyOptions && kittyOptions.mode !== 'disabled') {
        const flags = kittyOptions.flags ?? ['disambiguateEscapeCodes'];
