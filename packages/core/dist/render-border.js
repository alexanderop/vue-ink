...
const stylePiece = (segment, { color, bg, dim }) => {
...
const renderBorder = (x, y, node, output) => {
    ...
    const resolve = (color, bg, dim, show) => ({
    ...
    const horizontalEdge = (edge, l, mid, r) => stylePiece((left.show ? l : '') + mid.repeat(contentWidth) + (right.show ? r : ''), edge);
    const verticalEdge = (edge, glyph) => `${stylePiece(glyph, edge)}\n`.repeat(verticalHeight);
    const offsetY = top.show ? 1 : 0;
    if (top.show)
        output.write(x, y, horizontalEdge(top, box.topLeft, box.top, box.topRight), { transformers: [] });
    if (left.show)
        output.write(x, y + offsetY, verticalEdge(left, box.left), { transformers: [] });
    if (right.show)
        output.write(x + width - 1, y + offsetY, verticalEdge(right, box.right), { transformers: [] });
    if (bottom.show)
