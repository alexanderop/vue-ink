...
const squashTextNodesPlain = (node) => {
    const parts = [];
    for (const childNode of node.childNodes) {
        ...
        if (childNode.nodeName === 'ink-text' ||
            ...
            parts.push(squashTextNodesPlain(childNode));
        }
    }
    return parts.join('');
};
...
export const renderNodeToScreenReaderOutput = (node, options = {}) => {
    ...
    if (node.yogaNode?.getDisplay() === Yoga.DISPLAY_NONE)
        return '';
    if (node.internal_accessibility?.hidden)
        return '';
    let output = '';
    ...
    if (label !== undefined) {
        ...
        const childNodes = direction === 'row-reverse' || direction === 'column-reverse'
...
const renderNodeToOutput = (node, output, options) => {
    const { offsetX = 0, offsetY = 0, transformers = [], skipStaticElements = false, } = options;
