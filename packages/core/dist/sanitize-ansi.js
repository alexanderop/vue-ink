...
const CSI_PARAM = (code) => code >= 0x30 && code <= 0x3f;
const CSI_INTERMEDIATE = (code) => code >= 0x20 && code <= 0x2f;
const CSI_FINAL = (code) => code >= 0x40 && code <= 0x7e;
...
const sanitizeAnsi = (text) => {
    ...
    for (let i = 0; i < text.length; i += 1) {
        ...
        if (ch === '\x1B' && text[i + 1] === '[') {
            let j = i + 2;
            while (j < text.length && CSI_PARAM(text.charCodeAt(j)))
                j += 1;
            while (j < text.length && CSI_INTERMEDIATE(text.charCodeAt(j)))
                j += 1;
