...
// xterm/OSC BEL termination behavior:
...
const getControlStringFromEscapeIntroducer = (
	character: string,
): ControlStringIntroducer | undefined => {
	switch (character) {
		case ']':
...
export const hasAnsiControlCharacters = (text: string): boolean => {
	...
	for (const character of text) {
		if (isC1ControlCharacter(character)) return true;
	}
	return false;
};
