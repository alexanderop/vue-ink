...
const renderBackground = (x: number, y: number, node: DOMElement, output: Output): void => {
	const { style, yogaNode } = node;
	if (!style.backgroundColor || !yogaNode) return;

	const inset = (edge: boolean | undefined): number => (style.borderStyle && edge !== false ? 1 : 0);
