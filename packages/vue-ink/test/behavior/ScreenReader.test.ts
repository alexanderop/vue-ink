...
describe('screen-reader', () => {
	...
	it('render aria-label only Text for screen readers', () => {
		expect(sr(() => h(Text, { 'aria-label': 'Screen-reader only' }))).toBe(
			'Screen-reader only',
		);
	});

	it('render aria-label only Box for screen readers', () => {
		expect(sr(() => h(Box, { 'aria-label': 'Screen-reader only' }))).toBe(
			'Screen-reader only',
		);
	});
