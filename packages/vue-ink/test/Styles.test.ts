...
import { createNode, applyStyles } from "@vue-ink/core";
...
describe("styles", () => {
  it("applies margin via shorthand and per-edge", async () => {
    const Demo = defineComponent({
      setup: () => () => h(Box, { marginLeft: 2, marginTop: 1 }, () => h(Text, null, () => "x")),
    });
  ...
  it("applies margin shorthand (all edges)", async () => {
    const Demo = defineComponent({
      setup: () => () => h(Box, { margin: 1 }, () => h(Text, null, () => "a")),
    });
    const out = await renderToString(Demo, { columns: 10 });
  ...
  it("applies padding shorthand and per-edge", async () => {
    const Demo = defineComponent({
      setup: () => () => h(Box, { padding: 1, paddingLeft: 2 }, () => h(Text, null, () => "p")),
    });
    const out = await renderToString(Demo, { columns: 10 });
  ...
  it("applies paddingX, paddingY, paddingRight, paddingTop, paddingBottom", async () => {
    const Demo = defineComponent({
      setup: () => () =>
        h(Box, { paddingX: 1, paddingY: 1, paddingRight: 1, paddingTop: 1, paddingBottom: 1 }, () =>
          h(Text, null, () => "q"),
        ),
    });
    const out = await renderToString(Demo, { columns: 10 });
  ...
  it("respects flexBasis as a number", async () => {
    const Demo = defineComponent({
      setup: () => () =>
        h(Box, { width: 10 }, () => h(Box, { flexBasis: 5 }, () => h(Text, null, () => "x"))),
    });
  ...
  it("respects flexBasis as a percentage string", async () => {
    const Demo = defineComponent({
      setup: () => () =>
        h(Box, { width: 10 }, () => h(Box, { flexBasis: "50%" }, () => h(Text, null, () => "x"))),
    });
    const out = await renderToString(Demo, { columns: 20 });
    expect(out).toContain("x");
  });
  ...
  it("respects flexDirection variants", async () => {
    for (const flexDirection of ["row", "row-reverse", "column", "column-reverse"] as const) {
      const Demo = defineComponent({
        setup: () => () =>
          h(Box, { flexDirection }, () => [h(Text, null, () => "a"), h(Text, null, () => "b")]),
      });
      const out = await renderToString(Demo, { columns: 10 });
      expect(out).toContain("a");
    }
  ...
  it("respects alignItems variants", async () => {
    for (const alignItems of ["flex-start", "center", "flex-end", "stretch", "baseline"] as const) {
      const Demo = defineComponent({
        setup: () => () =>
          h(Box, { flexDirection: "column", height: 3, alignItems }, () =>
            h(Text, null, () => "a"),
          ),
      });
      const out = await renderToString(Demo, { columns: 10 });
  ...
  it("respects minWidth/minHeight as number and percent", async () => {
    const Demo = defineComponent({
      setup: () => () =>
        h(Box, { width: 10, flexDirection: "column" }, () =>
          h(Box, { minWidth: 5, minHeight: 2 }, () => h(Text, null, () => "a")),
        ),
    });
    const out = await renderToString(Demo, { columns: 20 });
    expect(out).toContain("a");
  ...
  it("respects maxWidth/maxHeight as number and percent", async () => {
    const Demo = defineComponent({
      setup: () => () => h(Box, { maxWidth: 4, maxHeight: 2 }, () => h(Text, null, () => "hello")),
    });
    const out = await renderToString(Demo, { columns: 20 });
